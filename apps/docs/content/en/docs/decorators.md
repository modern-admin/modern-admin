...
interface ResourceOptions {
  ...
  navigation?: { name?: string; icon?: string } // optional sidebar grouping
...
`navigation.name` groups resources in the sidebar. Use `navigation: {}` or
omit `name` to keep the resource at the top level; when `icon` is omitted the
default database icon is used.

`isVisible` accepts either a boolean (applies to every view) or per-view
toggles for fine-grained control. `isAccessible` evaluates on the server with
the current admin in `ctx.currentAdmin` and removes the property from metadata
and built-in action responses when it returns `false`. `components.*` strings
reference names registered with the `ComponentLoader`.
