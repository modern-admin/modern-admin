...
async function seedDemo(): Promise<void> {
  ...
  // a row in this table.
  //
  // Reserved synthetic keys in the permission matrix (recognised by the
  // framework, not real resources):
  //   • `__sql__: ['execute']` — grants the AI assistant's `execute_sql`
  //     tool to this role. Wildcards (`'*': ['*']` on `admin`) cover it
  //     for free; least-privilege roles like `viewer` intentionally lack
  //     it, so the AI cannot bypass per-resource gates via raw SQL when
  //     a viewer chats with it.
  const BUILTIN_ROLES = [
    {
      id: 'admin',
