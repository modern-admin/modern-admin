...
// every flavour does identically: extended query parser, static `/uploads`,
// CORS, and mounting Better Auth's Express handler at `/api/auth`. The
// only host-specific work happens before listen — usually migrations
// and seed steps — supplied via `preBootstrap`.
 ...
 *   3. Switch the Express query parser to `extended` so bracket-notation
 *      filter params (`filters[role]=admin`) are nested objects by the
 *      time they reach the action layer.
 *   4. Serve `<cwd>/uploads` under `/uploads` for the file feature.
 *   5. Enable CORS using `WEB_ORIGIN` (csv) or wide-open in dev.
 *   6. Mount Better Auth's Node handler at `/api/auth/*` BEFORE any body
 *      parsers can consume the request stream Better Auth needs raw.
 *   7. Listen on `API_PORT` / `API_HOST`.
 */
