          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0 space-y-1">
                <p className="truncate text-sm font-medium">{item.userId ?? l.unknownUser}</p>
                <p className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                  ...
                  {item.recordId && (
                    <code className="rounded bg-muted px-1.5 py-0.5 text-xs">#{item.recordId}</code>
                  )}
            ...
            <time className="shrink-0 text-xs text-muted-foreground">{formatDate(item.at)}</time>
          </div>
        </li>
      ))}
    </ol>
