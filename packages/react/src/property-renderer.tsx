...
export const formatMoneyValue = (value: unknown, currency?: string, locale?: string): string => {
...
export function PropertyDisplay({
  ...
  view = 'list',
  populated,
}: PropertyDisplayProps): React.ReactElement | null {
  ...
  switch (property.type) {
    ...
    case 'money': {
      const currency =
        typeof property.custom?.currency === 'string' ? property.custom.currency : undefined
      return withCopy(
        view === 'list' ? (
    ...
    case 'm2m': {
      ...
      const m2m = property.custom?.m2m as { reference: string; extraFields?: string[] } | undefined
...
function FilePropertyEditor({
  ...
  const startUploads = async (files: File[]): Promise<void> => {
    ...
    await client.uploadFiles(resourceId, property.path, accepted, {
      ...
      onItemError: (i, _f, err) => {
        setPending((prev) =>
          prev.map((p) => (p.id === ids[i] ? { ...p, status: 'error', error: err.message } : p)),
        )
      },
    })
  }
  ...
  const activePending =
    pending.find((p) => p.status === 'uploading') ?? pending.find((p) => p.status === 'queued')
          ...
          {items.map((item) => (
            <div key={item.id} className="rounded-md border border-border bg-muted/30 px-2.5 py-2">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:gap-3">
                ...
                <div
                  className={
                    'grid min-w-0 flex-1 gap-2 ' + (extras.length > 1 ? 'sm:grid-cols-2' : '')
                  }
                >
                  {extras.map((f) => {
  ...
  const dynamic = React.useMemo(
    () =>
      fields.filter(
        (
          f,
        ): f is KeyValueFieldSpec & {
          suggestionsResource: string
          suggestionsField: string
        } => f.type === 'autocomplete' && !!f.suggestionsResource && !!f.suggestionsField,
      ),
    [fields],
  )
