...
Commands:
  create-modern-admin <name> [--target ./dir]   Scaffold a new project.
...
const parse = (argv: string[]): Args => {
  ...
  //   create-modern-admin <name>     → scaffold
  //   modern-admin generate ...      → generate
  const head = argv[0]!
  if (head === '--help' || head === '-h') return { command: 'help' }
  if (head === 'generate') return parseGenerate(argv.slice(1))

  // Default: scaffold (matches the pre-existing `create-modern-admin <name>`
  // entry point so existing docs keep working unchanged).
