...
/**
 * Synthetic resource id used to gate the AI assistant's `execute_sql`
 * tool through the same role-permission matrix as everything else. A
 * role grants raw SQL access by listing `'execute'` (or `'*'`) under
 * this key, e.g. `{ '__sql__': ['execute'] }`. Wildcard role grants
 * (`{ '*': ['*'] }`, used by the seeded `admin` role) match it for
 * free; least-privilege roles must opt in explicitly.
 *
 * Kept out of `BUILT_IN_ACTIONS` and out of any real resource id so it
 * cannot collide with a user-defined resource.
 */
export const AI_SQL_PERMISSION_RESOURCE = '__sql__'
export const AI_SQL_PERMISSION_ACTION = 'execute'
...
export class AiAssistantService {
  ...
  async runChatJob(data: AiAssistantChatJobData): Promise<AiAssistantTaskOutput> {
    ...
    // Per-request gate on `execute_sql`. The host wires `rawQuery` once
    // at module-config level, but whether THIS admin's role is allowed
    // to use it depends on the role-permission matrix — checked here so
    // a less-privileged role chatting with the AI cannot bypass per-
    // resource gates by going straight to SQL.
    const rawQueryForRequest = await this.resolveRawQueryForRequest(data.currentAdmin, debug)
  ...
  private buildSystemPrompt(
    ...
    for (const descriptor of descriptors) {
      // Skip synthetic descriptors (`__sql__` for execute_sql, `__ui__` for
      // navigate_to). They are not real resources — listing them here would
      // make the model think it can `list __sql__` / `list __ui__`.
      if (descriptor.resourceId.startsWith('__')) continue
    ...
    if (sqlToolAvailable && sqlResources.length > 0) {
  ...
  /**
   * Decide whether the `execute_sql` tool may be wired for this request.
   * Returns the configured `rawQuery` when the current admin's role
   * grants `__sql__:execute` (matrix wildcards apply); returns
   * `undefined` otherwise so `buildAiAssistantTools` skips registering
   * the tool entirely. Default-deny: missing role data → no SQL.
   */
  private async resolveRawQueryForRequest(
    currentAdmin: CurrentAdmin | undefined,
    debug: boolean,
  ): Promise<((sql: string) => Promise<unknown[]>) | undefined> {
    const rawQuery = this.options.aiAssistant?.rawQuery
    if (!rawQuery) return undefined
    const role = currentAdmin?.role
    if (typeof role !== 'string' || role.length === 0) {
      if (debug) {
        this.logger.debug(
          'AI execute_sql gated off: current admin has no role; raw SQL requires explicit ' +
            `${AI_SQL_PERMISSION_RESOURCE}:${AI_SQL_PERMISSION_ACTION} grant.`,
        )
      }
      return undefined
    }
    const perms = await this.admin.getRolePermissions(role)
    if (!perms) {
      if (debug) {
        this.logger.debug(
          `AI execute_sql gated off: role "${role}" has no permission row ` +
            `(rolesResourceId configured? ${Boolean(this.admin.options.rolesResourceId)}).`,
        )
      }
      return undefined
    }
    if (!permissionsAllow(perms, AI_SQL_PERMISSION_RESOURCE, AI_SQL_PERMISSION_ACTION)) {
      if (debug) {
        this.logger.debug(
          `AI execute_sql gated off: role "${role}" lacks ` +
            `${AI_SQL_PERMISSION_RESOURCE}:${AI_SQL_PERMISSION_ACTION} grant.`,
        )
      }
      return undefined
